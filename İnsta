...
def rest(user):
    try:
        ...
        apoporno = response.get('email', 'Reset None')
    except:
        apoporno = 'Reset None'
    return apoporno
...
def InfoAcc(username, domain):
    ...
    user_id = account_info.get('pk') or 0
    full_name = account_info.get('full_name') or ''
    followers = account_info.get('follower_count') or 0
    following = account_info.get('following_count') or 0
    posts = account_info.get('media_count') or 0
    bio = account_info.get('biography') or ''
...
𝐌𝐄𝐓𝐀 : {meta_status}
𝐑𝐄𝐒𝐓 : {rest(username)}
𝐋𝐈𝐍𝐊 : https://instagram.com/{username}
══════════════════
"""

    # Dosyaya yaz
    with open('apohit.txt', 'a', encoding='utf-8') as f:
        f.write(info_text + "\n")

    photo_url = 'https://imgbox.com/3DmjpBKJ'  # Buraya fotoğraf URL'si
    ...
    try:
        ...
        print("[Telegram] Mesaj başarıyla gönderildi.")
    except Exception as e:
        print(f"[Telegram HATASI] Mesaj gönderilemedi: {e}")
